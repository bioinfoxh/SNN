...
def plot_mean_ROC_curves(mlp_ROC_points, rnn_ROC_points, snn_ROC_points):
    ...
    plt.plot([0, 1], [0, 1], linestyle='--', lw=lw, color='k', label='Random (AUC = $0.5$)')

    # MLP
    mlp_mean_tpr /= len(keys)
    mlp_mean_tpr[-1] = 1.0
    mlp_mean_auc = auc(mlp_mean_fpr, mlp_mean_tpr)
    plt.plot(mlp_mean_fpr, mlp_mean_tpr, color='green', label='MLP (AUC = $%0.2f$)' % mlp_mean_auc, lw=lw)

    # RNN
    rnn_mean_tpr /= len(keys)
    rnn_mean_tpr[-1] = 1.0
    rnn_mean_auc = auc(rnn_mean_fpr, rnn_mean_tpr)
    plt.plot(rnn_mean_fpr, rnn_mean_tpr, color='purple', label='RNN (AUC = $%0.2f$)' % rnn_mean_auc,
             lw=lw)
    ...
    plt.plot(snn_mean_fpr, snn_mean_tpr, color='red', label='SNN (AUC = $%0.2f$)' % snn_mean_auc,
             lw=lw)
    ...
    plt.title('Mean ROC curves after 10-fold crossvalidation', size=16, fontweight='bold')

    fig.savefig("../TSNE_plots/mean_ROC_cancer_data.pdf")
