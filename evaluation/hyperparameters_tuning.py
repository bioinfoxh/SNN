...
keep_probability_values = [0.8,0.6]
learning_rate_values = [ 0.01, 0.001]
weight_decay_values = [0.05, 0.0005] 

'''keep_probability_values = [0.8, 0.9]
learning_rate_values = [ 0.01, 0.1, ]
weight_decay_values = [0.0005, 0.005]'''


def choose_hyperparameters(network, k_fold_datasets):

    keep_probability_tuning = choose_keep_probability(network, k_fold_datasets, 0.01, 0.0005)
    keep_probability = keep_probability_tuning[0]
    
    weight_decay_tuning = choose_weight_decay(network, k_fold_datasets, 0.01, keep_probability)
    weight_decay = weight_decay_tuning[0]
    
    learning_rate_tuning = choose_learning_rate(network, k_fold_datasets, weight_decay, keep_probability)
    learning_rate = learning_rate_tuning[0]
