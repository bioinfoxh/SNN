...
class SuperlayeredNeuralNetwork(object):
    ...
    def compute_predictions(self, s1_input_data, s2_input_data, weights, biases, keep_probability):
        ...
        s2_input_to_third_hidden_layer = \
            ...
            tf.matmul(s1_second_hidden_layer, weights['cross_connection_from_s1']) + \
    ...
    def plotSNE(self, s1_before, s2_before, s1_cross_connection, s2_cross_connection, merge_layer, colors):
        ...
        ax.scatter(x_green, y_green, label='normal', c=colors, cmap=plt.get_cmap('Spectral'))
        colors = ['purple'] * len(x_purple)
        ax.scatter(x_purple, y_purple, label='cancer', c=colors, cmap=plt.get_cmap('Spectral'))
        ...
        ax.legend(loc='lower center', bbox_to_anchor=(0.5, 2))

        fig.savefig("t-sne.pdf")
