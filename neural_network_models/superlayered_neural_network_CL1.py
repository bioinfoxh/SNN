...
class SuperlayeredNeuralNetwork(object):
    ...
    def initialize_weights_and_biases_for_superlayered_network(
        ...
        weights['cross_connection_from_s1'] = tf.Variable(
            tf.truncated_normal([s1_hidden_units[0], s2_hidden_units[1]],
                                stddev=math.sqrt(2.0 / float(s1_hidden_units[0]))))

        weights['cross_connection_from_s2'] = tf.Variable(
            tf.truncated_normal([s2_hidden_units[0], s1_hidden_units[1]],
                                stddev=math.sqrt(2.0 / float(s2_hidden_units[0]))))
    ...
    def compute_predictions(self, s1_input_data, s2_input_data, weights, biases, keep_probability):
        ...
        """ Second hidden layer that contains cross-connection between the superlayers """

        s1_input_to_second_hidden_layer = \
            tf.matmul(s1_first_hidden_layer, s1_weights['weights_first_hidden_layer']) + \
            tf.matmul(s2_first_hidden_layer, weights['cross_connection_from_s2']) + \
        ...
        s2_input_to_second_hidden_layer = \
            tf.matmul(s2_first_hidden_layer, s2_weights['weights_first_hidden_layer']) + \
            tf.matmul(s1_first_hidden_layer, weights['cross_connection_from_s1']) + \
            s2_biases['biases_second_hidden_layer']
        ...
        s2_input_to_third_hidden_layer = \
            tf.matmul(s2_second_hidden_layer, s2_weights['weights_second_hidden_layer']) + s2_biases[
                'biases_third_hidden_layer']
