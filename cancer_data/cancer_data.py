...
from cancer_data_processing import create_k_fold_patient_ids, create_k_fold_datasets, create_k_fold_datasets_with_clusters, \
    extract_patients_data, create_one_hot_encoding, create_label_to_patient_ids
...
class CancerPatientsData(EpigeneticData):
    ...
    def extract_cancer_patients_data(self):
        cancer_data_file = open('/Users/Hui/myFiles/cambridge/projects/SNN/code/TFv1_xh/datasets/tnf_fpkm_body_snn.txt', 'r')
...
class CancerPatientsDataMethylationLevels(CancerPatientsData):
...
class CancerPatientsDataExpressionLevels(CancerPatientsData):

    def __init__(self, num_folds, num_folds_hyperparameters_tuning):
        CancerPatientsData.__init__(self, num_folds, num_folds_hyperparameters_tuning)

        patient_ids = self.patient_id_to_label.keys()
        self.input_size = len(self.patient_id_to_gene_expressions[patient_ids[0]])
        print (self.input_size)
    ...
    def get_k_fold_datasets(self):
        ...
        self.k_fold_datasets = create_k_fold_datasets(
            self.num_folds, k_fold_patient_ids, self.input_size, self.output_size,
            self.patient_id_to_gene_expressions, self.label_to_one_hot_encoding,
            self.patient_id_to_label)
    ...
    def add_Gaussian_noise(self, mean, stddev):
        patient_ids = self.patient_id_to_gene_expressions.keys()
        for patient_id in patient_ids:
            patient_data = self.patient_id_to_gene_expressions[patient_id]
            for index in range(len(patient_data)):
                patient_data[index] = float(patient_data[index]) + np.random.normal(mean, stddev)
            self.patient_id_to_gene_expressions[patient_id] = patient_data
